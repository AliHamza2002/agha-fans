...
export function AddBuyerModal({ isOpen, onClose, onSave, finishedMaterials }: AddBuyerModalProps) {
	...
	const [items, setItems] = useState<ItemRow[]>([{ id: 1, itemName: '', itemPrice: 0 }]);
	...
	useEffect(() => {
		if (isOpen) {
			...
			setContact('');
			setAddress('');
			setItems([{ id: 1, itemName: finishedMaterials[0]?.name || '', itemPrice: 0 }]);
			setNextId(2);
		}
	...
	const calculateTotal = () => {
		return items.reduce((sum, item) => sum + (item.itemPrice || 0), 0);
	};
	...
	const handleSubmit = () => {
		...
		// Validate items
		const validItems = items.filter(item => item.itemName.trim() && item.itemPrice > 0);
