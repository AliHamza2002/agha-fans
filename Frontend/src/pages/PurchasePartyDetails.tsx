...
export default function PurchasePartyDetails() {
		...
		return (
			<div className="space-y-6">
				<div className="bg-white rounded-2xl p-8 shadow-soft border border-slate-200 text-center">
					<p className="text-slate-600 mb-4">Supplier not found.</p>
					<button
	...
	// Ensure this is a Supplier party
	if (party.type !== 'Supplier') {
		return (
			<div className="space-y-6">
				<div className="bg-white rounded-2xl p-8 shadow-soft border border-slate-200 text-center">
					<p className="text-slate-600 mb-4">This party is not a supplier.</p>
					<button
						onClick={() => navigate('/purchase')}
						className="inline-flex items-center gap-2 px-4 py-2 bg-brand text-white rounded-xl hover:bg-brand-hover transition"
					>
	...
	return (
		<div className="space-y-6">
			...
			<div className="bg-white rounded-2xl p-6 shadow-soft border border-slate-200">
				<h2 className="text-xl font-bold text-slate-900 mb-4">Supplier Information</h2>
				<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
					<div>
						<p className="text-xs font-medium text-slate-600 mb-1">Supplier Name</p>
			...
			<div className="bg-white rounded-2xl p-5 shadow-soft border border-slate-200">
				<h2 className="text-lg font-semibold text-slate-900 mb-4">Purchase Ledger</h2>
				{partyTransactions.length === 0 ? (
					<div className="text-center py-12">
						<p className="text-slate-500">No transactions found for this supplier.</p>
					</div>
				) : (
								...
								{partyTransactions.map(t => (
									<tr key={t.id} className="border-b border-slate-100 hover:bg-indigo-50/50 transition">
										...
										<td className="p-3 text-slate-900 text-left">
											{t.type === 'Payment' ? (
												<span className="font-medium text-emerald-700">{t.notes || 'Payment'}</span>
											) : (
												<span>
										...
										<td className="p-3 text-slate-600 text-center">
											{t.type === 'Payment' ? '-' : t.unitPrice.toLocaleString()}
										</td>
										<td className="p-3 font-medium text-slate-900 text-center">
