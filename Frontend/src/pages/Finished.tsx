					...
					<tbody>
						{finishedMaterials.map(m => (
							<tr key={m.id} className="border-b border-slate-100 hover:bg-indigo-50/50 transition">
								...
								{isAdmin && <td className="p-3 text-slate-600">Rs. {(m.unitPrice || 0).toLocaleString()}</td>}
								{isAdmin && <td className="p-3 font-semibold text-slate-900">Rs. {((m.unitPrice || 0) * m.quantity).toLocaleString()}</td>}
								<td className="p-3 text-slate-600 text-xs">{m.description || '-'}</td>
								<td className="p-3">
